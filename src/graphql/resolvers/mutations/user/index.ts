...
export async function deleteUser(_, __, ctx: Context) {
  ...
  if (viewer.isAdmin) {
    throw new GraphQLError('Admins can’t be deleted', { extensions: { code: 'BAD_USER_INPUT' } })
  }
...
export async function editUser(_, args: MutationEditUserArgs, ctx: Context) {
  ...
  if (username) {
    if (!validUsername(username)) {
      throw new GraphQLError('Usernames can be 16 characters long', { extensions: { code: 'BAD_USER_INPUT' } })
    }
    ...
    if (user && user.id !== viewer.id) {
      throw new GraphQLError('That username is taken', { extensions: { code: 'BAD_USER_INPUT' } })
    }
  ...
  if (email) {
    if (!validEmail(email)) {
      throw new GraphQLError('That email is not valid', { extensions: { code: 'BAD_USER_INPUT' } })
    }
    ...
    if (userByEmail && userByEmail.id !== viewer.id) {
      throw new GraphQLError('That email is taken', { extensions: { code: 'BAD_USER_INPUT' } })
    }
