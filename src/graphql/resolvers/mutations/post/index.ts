...
export async function editPost(_, args: MutationEditPostArgs, ctx: Context) {
  ...
  if (existing?.id !== id) throw new GraphQLError('Slug already exists', { extensions: { code: 'BAD_USER_INPUT' } })
  ...
  return await prisma.post
    ...
    .catch((err) => {
      console.error({ err })
      throw new GraphQLError('Unable to edit post', { extensions: { code: 'BAD_USER_INPUT' } })
    })
}
...
export async function addPost(_, args: MutationAddPostArgs, ctx: Context) {
  ...
  return await prisma.post
    ...
    .catch((err) => {
      console.error({ err })
      throw new GraphQLError('Unable to add post', { extensions: { code: 'BAD_USER_INPUT' } })
    })
}
